...
- Proves: schema validity, ssot_id format (scenario-hash), two-state route fixtures, legacy `/1` worker-result compatibility (`agent_type` is canonical, `role` remains accepted only for older payloads), builder work-package binding, handoff dependency/identity validation, evidence-requirement satisfiability, blocked/partial recovery regressions, fail-closed Builder partial checkpoint schema/fixture coverage (`changeset` + `resume_from` + verification state), and fixture invariants (no write ownership overlap).
