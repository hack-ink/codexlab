...
- Treat `state.phase = "done"` as plan-local completion only. It means this saved plan's task graph is terminal; it does not by itself certify downstream review, merge, `delivery-closeout`, or worktree cleanup unless those lifecycle stages are explicit tasks in the same saved plan.
