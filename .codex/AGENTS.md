...
- For implementation work, route into `worktrees` first unless the task is read-only or the correct lane is already active.
...
- When a task does use a persisted `plan/1`, keep that artifact inside the active worktree lane instead of leaving task-local plan files behind in the primary checkout.
...
- Use `analyst` for read-only evaluation of one option, hypothesis, or scenario at a time when multiple candidates can be assessed independently.
- Create child-agent objectives only when each objective is distinct and its output will be consumed by the main thread.
- Scout, skeptic, and analyst child agents must not edit repo content, delegate further, or claim implementation ownership.
- Use one bounded child-agent checkpoint round at a time: when child agents are warranted, dispatch distinct objectives, keep executing directly, do one bounded collect step at the next decision boundary, treat a timed-out collect as not ready yet, and retire stale child agents once enough evidence exists.
...
- Scout, skeptic, and analyst child-agent capability boundaries are behavior rules, not skill-name allowlists.
...
- For questions about a GitHub repository outside the current checkout, use DeepWiki first when the repo is available there.
- For current-checkout implementation questions, inspect local files first; use DeepWiki only for upstream comparison or when local context is missing.
- If Context7 or DeepWiki is insufficient, fall back to official docs or direct repo inspection and say so briefly.

### Identity Routing

- Treat Git identity and Linear routing selection as machine-routable context, not guesswork.
...
- `x` -> person `xavier` -> GitHub token `GITHUB_PAT_X` -> Linear target `helixbox` -> MCP server `linear_helixbox`
- `y` -> person `yvette` -> GitHub token `GITHUB_PAT_Y` -> Linear target `hackink` -> MCP server `linear_hackink`
...
- When a task touches GitHub or Linear state, use the routed identity/target from these signals unless the user explicitly says otherwise.
...
- For parallel implementation lanes, prefer isolated worktree-backed lanes under a repo-local `.worktrees/` directory over clone-backed duplicate checkouts.
- Default one `.worktrees/*` lane per task, branch, and review stream.
- When multiple `.worktrees/*` lanes conflict, route to `worktree-reconcile` and keep one surviving lane.
- After a task is truly complete, return to `worktrees` for cleanup of the worktree plus local and remote branches.
...
- Default development flow is: `worktrees` -> `review-prepare` -> `pr-land` -> `delivery-closeout` -> `worktrees` cleanup.
...
- If a task uses a persisted `plan/1`, create, revise, and execute it from inside the active worktree lane so planning and implementation stay together.
...
- `plan/1` completion is plan-local. A saved plan reaching `done` does not by itself bypass downstream review, merge, `delivery-closeout`, or worktree cleanup gates unless those lifecycle stages are explicit tasks in that same saved plan.
- If implementation hits a blocker, structural change, or architecture result that changes task shape, route back through `plan-writing` before continuing only when the task is already using a saved `plan/1`; otherwise re-scope directly from the active worktree and research result.
...
- `pr-land` stops at merge. It must not swallow `delivery-closeout` or worktree cleanup.
...
- Treat worktree cleanup the same way: no cleanup before the review gate and merge/closeout state are actually satisfied.
